...
Todo se apoya en las vistas de ``38_cambios_neumaticos.sql``. Acá no se
...
llevaba cada día, y eso sale de la tabla ``enganches``, que se maneja
desde Flota → Asociación de equipos (ver ``enganches.py``). Acá solo se lee.
"""
import datetime
...
def _exigir_instalado(cx):
    if not instalado(cx):
        raise ValueError("Todavía no está prendido el aviso de cambios. Hay que "
                         "correr gomeria/38_cambios_neumaticos.sql en Supabase.")
...
def panel(cx):
    ...
    if not instalado(cx):
        return {"instalado": False,
                "aviso": "Falta correr gomeria/38_cambios_neumaticos.sql en Supabase."}

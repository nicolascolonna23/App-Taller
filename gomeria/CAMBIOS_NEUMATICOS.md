...
Se prende corriendo **`28_cambios_neumaticos.sql`** en Supabase → SQL
...
falta saber qué tractor lo llevaba **cada día**, no solo hoy. Eso vive en
la tabla `enganches`:

- **Se llena sola.** Cada vez que se cambia el semi asociado en la ficha
  del tractor (Flota), la base cierra el enganche anterior y abre el
  nuevo. No hay que acordarse de nada.
- **El día que se corre el SQL** se toma el enganche de hoy y se supone
  que viene desde la primera lectura del satelital de ese tractor. Si no
  fue así, se corrige la fecha desde la pantalla, o se cargan los
  enganches anteriores (sección *Qué tractor llevó a cada semi*).

Si algún semi tiene satelital propio, gana su lectura.
...
| Tabla / vista | Qué es |
|---|---|
| `enganches` | qué tractor llevó a cada semi y entre qué fechas |
...
- Reglas, anular un cambio y corregir enganches: encargado o administrador.

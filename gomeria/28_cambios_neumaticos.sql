...
-- Tres piezas:
--
--   1. EL ENGANCHE. Un semi no tiene odómetro: anda lo que anda el
--      tractor que lo lleva. La ficha del tractor dice qué semi lleva
--      HOY, pero para contar kilómetros hace falta saber cuál llevaba
--      cada día. Esta tabla guarda esa historia, y se llena sola cada vez
--      que alguien cambia el semi asociado en la ficha.
--
...
--
-- Necesita 01, 05 (odómetros) y 07 (maestro de unidades) corridos.
-- =====================================================================


-- ---------------------------------------------------------------------
-- 1. EL ENGANCHE: qué tractor llevaba a cada semi, y desde cuándo
-- ---------------------------------------------------------------------
-- Intervalo semiabierto: el semi anduvo con ese tractor desde 'desde'
-- (inclusive) hasta 'hasta' (exclusive). Abierto (hasta null) es hoy.
create table if not exists enganches (
  ...
  hasta       date,
  -- ficha   = lo anotó la base sola, al cambiar el semi en la ficha
  -- inicial = el enganche que había el día que se corrió este archivo
  -- manual  = cargado a mano, para completar la historia
  origen      text   not null default 'ficha'
              check (origen in ('ficha','inicial','manual')),
  usuario     text,
  creado      timestamptz not null default now(),
  check (hasta is null or hasta >= desde),
  check (semi_id <> tractor_id)
);

-- Un semi va enganchado a un solo tractor, y un tractor lleva un solo semi.
...
create index if not exists ix_enganches_semi    on enganches(semi_id, desde);
create index if not exists ix_enganches_tractor on enganches(tractor_id, desde);

comment on table enganches is
  'Historia de qué tractor llevó a cada semi. De acá salen los km de los semis.';

-- La unidad que tiene esa patente, escrita como sea.
create or replace function _unidad_por_patente(p text) returns bigint as $$
  select id from unidades
  where regexp_replace(upper(patente), '[^A-Z0-9]', '', 'g')
...
-- Cada vez que cambia el semi de la ficha de un tractor, se cierra el
-- enganche anterior y se abre el nuevo. Nadie tiene que acordarse.
create or replace function _enganche_por_ficha() returns trigger as $$
declare
  v_semi bigint := _unidad_por_patente(new.semi);
begin
  if tg_op = 'UPDATE' and new.semi is not distinct from old.semi then
    return new;
  end if;
  if v_semi = new.id then
    v_semi := null;
  end if;
  -- Ya estaba enganchado a ese mismo: no hay nada que anotar.
  if v_semi is not null and exists (
       select 1 from enganches
       where tractor_id = new.id and semi_id = v_semi and hasta is null) then
    return new;
  end if;
  update enganches set hasta = greatest(desde, current_date)
   where tractor_id = new.id and hasta is null;
  if v_semi is not null then
    -- Si el semi venía con otro tractor, se lo sacaron a ese.
    update enganches set hasta = greatest(desde, current_date)
     where semi_id = v_semi and hasta is null;
    insert into enganches (semi_id, tractor_id, desde, origen)
    values (v_semi, new.id, current_date, 'ficha');
  end if;
  return new;
end $$ language plpgsql;

drop trigger if exists tg_enganche_por_ficha on unidades;
create trigger tg_enganche_por_ficha
  after insert or update of semi on unidades
  for each row execute function _enganche_por_ficha();

-- Lo que hay hoy. No se sabe desde cuándo cada semi va con su tractor, así
-- que se supone que desde siempre: desde la primera lectura del satelital
-- de ese tractor. Si no fue así, se corrige desde la pantalla cargando el
-- enganche anterior. Solo se agrega lo que falta.
insert into enganches (semi_id, tractor_id, desde, origen)
select s.id, t.id,
       coalesce((select min(o.fecha) from odometros o where o.unidad_id = t.id),
                current_date),
       'inicial'
from unidades t
join unidades s on s.id = _unidad_por_patente(t.semi) and s.id <> t.id
where t.activa
  and not exists (select 1 from enganches e where e.tractor_id = t.id and e.hasta is null)
  and not exists (select 1 from enganches e where e.semi_id   = s.id and e.hasta is null)
...
-- del tractor que lo llevaba ese día. Si algún semi tiene satelital
-- propio, gana su lectura: es un dato y no una deducción.
...
union all
select e.semi_id, k.fecha, k.recorrido, 'tractor', e.tractor_id
from enganches e
join v_km_diarios k
...
where k.recorrido is not null
  and not exists (select 1 from odometros o
                  where o.unidad_id = e.semi_id and o.fecha = k.fecha);
...
-- =====================================================================
alter table enganches          enable row level security;

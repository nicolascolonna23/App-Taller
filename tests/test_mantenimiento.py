...
class Planes(unittest.TestCase):
    ...
    def test_sin_datos_deja_la_patente_sin_plan(self):
        xml = '''<?xml version="1.0" encoding="UTF-8"?>
        <worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>
        <row r="1"><c r="A1" t="inlineStr"><is><t>PATENTE</t></is></c><c r="B1" t="inlineStr"><is><t>TIPO DE SERVICE</t></is></c></row>
        <row r="2"><c r="A2" t="inlineStr"><is><t>AD 247 MQ</t></is></c><c r="B2" t="inlineStr"><is><t>SIN DATOS</t></is></c></row>
        </sheetData></worksheet>'''
        archivo = io.BytesIO()
        with zipfile.ZipFile(archivo, "w") as z:
            z.writestr("xl/worksheets/sheet1.xml", xml)
        cx = BaseFalsa()
        salida = mantenimiento.importar(cx, archivo.getvalue())
        self.assertIn("AD 247 MQ", salida["sin_datos"])
        self.assertTrue(any(q.startswith("update unidades set mantenimiento_plan_id=null")
                            and v == (7,) for q, v in cx.consultas))

...
- Las vistas de 38_cambios_neumaticos.sql contra un PostgreSQL de prueba,
...
class Vistas(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Una base propia y no un esquema: varios scripts revisan que lo
        # anterior esté corrido mirando en public.
        cls.base = "test_neu_" + uuid.uuid4().hex
        with psycopg.connect(os.environ["MT_TEST_DATABASE_URL"], autocommit=True) as cx:
            cx.execute("create database " + cls.base)
        cls.url = make_conninfo(os.environ["MT_TEST_DATABASE_URL"], dbname=cls.base)
        cls.addClassCleanup(cls.limpiar)
        with cls.conectar() as cx:
            # Todo el esquema, en orden, hasta el anterior a este módulo:
            # los km del semi salen de los enganches de 29.
            for f in sorted((RAIZ / "gomeria").glob("[0-3][0-9]_*.sql")):
                if f.name < "38_":
                    cx.execute(f.read_text())
            cls.cargar_datos(cx)
            cx.execute((RAIZ / "gomeria" / "38_cambios_neumaticos.sql").read_text())
            cls.despues_del_sql(cx)
            cx.commit()

    @classmethod
    def conectar(cls):
        return cls.pg.connect(cls.url, row_factory=cls.row)

    @classmethod
    def limpiar(cls):
        with cls.pg.connect(os.environ["MT_TEST_DATABASE_URL"], autocommit=True) as cx:
            cx.execute("drop database if exists " + cls.base + " with (force)")
    ...
    def cargar_datos(cls, cx):
        ...
        for nombre in ("S-D-D", "D-D-D"):
            cfg = cx.execute("""insert into configuraciones(nombre) values (%s)
    ...
    def test_enganche_inicial_desde_la_primera_lectura(self):
        ...
        self.assertEqual(e[0]["usuario"], "sistema")
    ...
    def test_el_semi_deja_de_sumar_cuando_se_desengancha(self):
        with self.conectar() as cx:
            cx.execute("update enganches set hasta=%s where semi_id=%s", (self.dia(50), self.semi))
            f = self.eje(cx, self.semi, 1)
            cx.rollback()
        # Del día siguiente al cambio hasta el día que se bajó, inclusive.
        self.assertEqual(f["km"], 50 * 500)

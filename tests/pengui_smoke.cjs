...
(async()=>{
 ...
 try{
  ...
  await page.route('**/*',route=>{
   ...
   if(p==='/api/services')return route.fulfill({json:[{unidad_id:7,patente:'AG797NJ',interno:'102',sucursal:'BUE',plan_id:1,plan_nombre:'Autos 10K',ultimo_fecha:'2026-09-01',ultimo_km:41259,cada_km:10000,proximo_km:51259,km_actual:41833,km_restantes:9426,estado:'proximo'},{unidad_id:9,patente:'AH522SI',interno:'17',sucursal:'LAD',plan_id:2,plan_nombre:'S-WAY',ultimo_fecha:'2026-08-01',ultimo_km:150000,cada_km:45000,proximo_km:195000,km_actual:170000,km_restantes:25000,estado:'ok'}]});
   if(p==='/api/mantenimiento')return route.fulfill({json:{planes:[{id:1,nombre:'Autos 10K',descripcion:'Autos',cada_km:10000,activo:true,unidades:1},{id:2,nombre:'S-WAY',descripcion:'Camiones S-WAY',cada_km:45000,activo:true,unidades:1}],asignaciones:[{unidad_id:7,patente:'AG797NJ',interno:'102',modelo:'HIACE',planes:[1]},{unidad_id:9,patente:'AH522SI',interno:'17',modelo:'S-WAY 480',planes:[2]}],modelos:[{modelo:'HIACE',unidades:1},{modelo:'S-WAY 480',unidades:1}]}});
  ...
  for(const url of Object.keys(files).filter(x=>x!=='/asistente')){
    ...
    if(url==='/control'){
     ...
     assert((await page.locator('#tbl-lad').innerText()).toUpperCase().includes('TIPO DE SERVICE'));

...
(() => {
  ...
  root.innerHTML = `<style>
    :host{position:fixed;bottom:max(16px,env(safe-area-inset-bottom));left:max(16px,env(safe-area-inset-left));z-index:900;font:13px system-ui;color:#172b40}
    *{box-sizing:border-box}button{font:inherit;cursor:pointer}button:focus-visible,a:focus-visible{outline:3px solid #3185cf;outline-offset:4px}
    .launch{display:flex;align-items:center;gap:3px;background:none;border:0;padding:0;color:inherit}
    svg{width:72px;height:86px;filter:drop-shadow(0 4px 4px #14263830);transform-origin:50% 90%}
    .name{background:#fff;padding:8px 12px;border:1px solid #dbe3eb;border-radius:20px;box-shadow:0 3px 12px #14263812;font-weight:650}
    ...
    .chat{position:absolute;bottom:98px;left:0;width:min(420px,calc(100vw - 32px));height:min(640px,calc(100dvh - 140px));display:flex;flex-direction:column;border-radius:16px;overflow:hidden;border:1px solid #dbe3eb;box-shadow:0 16px 60px #14263840;background:white}
    [hidden]{display:none!important}.head{display:flex;align-items:center;padding:13px 16px;background:#142638;color:white;gap:12px}.head div{flex:1}.head small{display:block;color:#c6d5e3;font-size:11px;margin-top:3px}.head button,.head a{border:0;background:transparent;color:white;padding:7px;text-decoration:none;font-size:20px;border-radius:6px}
    iframe{border:0;width:100%;flex:1;min-height:0;background:#f3f5f7}
  ...
  <button class="launch" aria-label="Hablar con Pengui, el asistente de IA" aria-expanded="false">
    <svg viewBox="0 0 100 120" aria-hidden="true">
      <ellipse cx="50" cy="112" rx="28" ry="5" fill="#142638" opacity=".12"/>
      <ellipse cx="33" cy="106" rx="16" ry="7" fill="#ed8900"/><ellipse cx="67" cy="106" rx="16" ry="7" fill="#ed8900"/>
      <path d="M25 51C4 58 7 88 17 83L30 68M75 51C93 40 100 48 86 64L75 73" fill="#203b53"/>
      <path d="M20 62C15 13 35 7 50 7S85 13 80 62C93 108 69 111 50 111S7 108 20 62" fill="#142638"/>
      ...
      <ellipse cx="39" cy="39" rx="4" ry="6" fill="#142638"/><ellipse cx="61" cy="39" rx="4" ry="6" fill="#142638"/>
      <circle cx="40" cy="37" r="1.5" fill="white"/><circle cx="62" cy="37" r="1.5" fill="white"/>
      <path d="M40 49Q50 43 60 49L50 59Z" fill="#ed8900"/>
      <path d="M24 61Q50 71 76 61L76 70Q50 80 24 70Z" fill="#ed8900"/><path d="M66 70L76 70L77 89L66 85Z" fill="#ed8900"/>
